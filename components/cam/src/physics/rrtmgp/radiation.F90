...
contains
   ...
   subroutine radiation_tend(state,   ptend,    pbuf,          cam_out, cam_in,  &
      ...
      real(r8), pointer, dimension(:,:) :: cloud_fraction, snow_fraction
      real(r8), dimension(pcols,pver) :: combined_cloud_fraction
      ...
      if (radiation_do('sw')) then
         ...
         call t_startf('rad_daynight_indices_sw')
         ...
         call t_stopf('rad_albedo_sw')

         ! Allocate shortwave fluxes and optics
         call t_startf('rad_initialize_objects_sw')
         ...
         call t_stopf('rad_initialize_objects_sw')
         ...
         do icall = N_DIAG,0,-1
            if (active_calls(icall)) then
               ...
               call get_cloud_optics_sw( &
                  cloud_fraction, iclwp, iciwp, &
                  mu, lambdac, dei, &
                  cld_optics_sw_byband, &
                  snow_fraction, icswp, des &
               )
               call set_cloud_optics_sw( &
                  state%pmid(1:ncol,1:pver), combined_cloud_fraction(1:ncol,1:pver), &
                  cld_optics_sw_byband, cld_optics_sw &
               )
               ...
               call t_startf('rad_heating_rates_sw')
               ...
               call t_stopf('rad_heating_rates_sw')

               ! Send fluxes and optics to history buffer
               call t_startf('rad_output_sw')
               call output_fluxes_sw(icall, state, fluxes_allsky, fluxes_clrsky, qrs,  qrsc)
               if (icall == 0) call output_cloud_optics_sw(state, cld_optics_sw_byband)
               call t_stopf('rad_output_sw')

            end if
         end do

         ! Export fluxes
         call t_startf('rad_export_fluxes_sw')
         call set_net_fluxes_sw(fluxes_allsky, fsds, fsns, fsnt)
         call export_surface_fluxes(fluxes_allsky, cam_out, 'shortwave')
         call t_stopf('rad_export_fluxes_sw')
         
         ! Free memory
         call t_startf('rad_finalize_objects_sw')
         ...
         call t_stopf('rad_finalize_objects_sw')

      else

         ! Conserve energy
         call t_startf('rad_convert_heating_rates')
      ...
      if (radiation_do('lw')) then
         ...
         call t_stopf('rad_initialize_objects_lw')
         ...
         do icall = N_DIAG,0,-1
            if (active_calls(icall)) then
               ...
               call get_cloud_optics_lw( &
                  cloud_fraction, iclwp, iciwp, &
                  mu, lambdac, dei, &
                  cld_optics_lw_byband, &
                  snow_fraction, icswp, des &
               )
               call set_cloud_optics_lw( &
                  state%pmid(1:ncol,1:pver), combined_cloud_fraction(1:ncol,1:pver), &
                  cld_optics_lw_byband, cld_optics_lw &
               )
               ...
               ! Calculate heating rates
               call t_startf('rad_heating_rates_lw')
               call calculate_heating_rate(                  &
                  fluxes_allsky%flux_up(1:ncol,ktop:kbot+1), &
                  fluxes_allsky%flux_dn(1:ncol,ktop:kbot+1), &
                  pint(1:ncol,ktop:kbot+1),                  &
         ...
         call t_stopf('rad_convert_heating_rates')

      end if  ! dolw

      ! Compute net radiative heating tendency
      call t_startf('rad_radheat_tend')
      call radheat_tend(state, pbuf, ptend, &
                        qrl, qrs, &
                        fsns, fsnt, flns, flnt, &
                        cam_in%asdir, net_flux)
      call t_stopf('rad_radheat_tend')
   ...
   subroutine calculate_fluxes_sw(pmid, tmid, pint, tint, &
      ...
      ! Allocate daytime-only flux and optics objects
      call t_startf('rad_initialize_daytime_objects')
      ...
      call t_stopf('rad_initialize_daytime_objects')
      ...
      call t_startf('rad_finalize_daytime_objects')
      ...
      call t_stopf('rad_finalize_daytime_objects')
   ...
   subroutine get_cloud_optics_sw( &
      ...
      real(r8), intent(in), dimension(:,:) :: &
         cloud_fraction, iclwp, iciwp, mu, lambdac, dei
      real(r8), intent(in), dimension(:,:), optional :: &
         snow_fraction, icswp, des
      ...
      ! output arrays.
      real(r8), dimension(nswbands,size(iclwp,1),size(iclwp,2)) :: &
            liq_tau, liq_ssa, liq_asm, ice_tau, ice_ssa, ice_asm, &
      ...
      real(r8) :: combined_fraction(size(iclwp,1),size(iclwp,2))

      integer :: ncol, nlev, ibnd, icol, ilev

      ncol = size(cloud_fraction, 1)
      nlev = size(cloud_fraction, 2)

      ! Get ice cloud optics
      call mitchell_ice_optics_sw( &
         ncol, nlev, iciwp, dei, &
         ice_tau, ice_ssa, ice_asm &
      )
      
      ! Get liquid cloud optics
      call conley_liquid_optics_sw(         &
         ncol, nlev, lambdac, mu, iclwp,   &
         liq_tau, liq_ssa, liq_asm          &
      )

      ! Get snow optics
      if (present(icswp) .and. present(des)) then
         call mitchell_ice_optics_sw( &
            ncol, nlev, icswp, des, &
            snw_tau, snw_ssa, snw_asm &
         )
      ...
      if (present(snow_fraction)) then
         ...
         ! Combine optical properties, weighting by cloud/snow fraction
         combined_fraction = max(cloud_fraction, snow_fraction)
         do ilev = 1,pver
            do icol = 1,ncol
               do ibnd = 1,nswbands
                  if (combined_fraction(icol,ilev) > 0) then
                     cld_tau(ibnd,icol,ilev) = ( &
                        cloud_fraction(icol,ilev) * cld_tau(ibnd,icol,ilev) &
                      + snow_fraction (icol,ilev) * snw_tau(ibnd,icol,ilev) &
                     ) / combined_fraction(icol,ilev)
                     cld_tau_ssa(ibnd,icol,ilev) = ( &
                        cloud_fraction(icol,ilev) * cld_tau_ssa(ibnd,icol,ilev) &
                      + snow_fraction (icol,ilev) * snw_tau_ssa(ibnd,icol,ilev) &
                     ) / combined_fraction(icol,ilev)
                     cld_tau_ssa_asm(ibnd,icol,ilev) = ( &
                        cloud_fraction(icol,ilev) * cld_tau_ssa_asm(ibnd,icol,ilev) &
                      + snow_fraction (icol,ilev) * snw_tau_ssa_asm(ibnd,icol,ilev) &
                     ) / combined_fraction(icol,ilev)
                  else
                     cld_tau(ibnd,icol,ilev) = 0
                     cld_tau_ssa(ibnd,icol,ilev) = 0
                     cld_tau_ssa_asm(ibnd,icol,ilev) = 0
                  end if
               end do
            end do
         end do
      ...
      ! albedo, and assymmetry parameter.
      do ibnd = 1,nswbands
         do ilev = 1,pver
            do icol = 1,ncol
               optics_out%tau(icol,ilev,ibnd) = cld_tau(ibnd,icol,ilev)
               if (cld_tau(ibnd,icol,ilev) > 0) then
                  optics_out%ssa(icol,ilev,ibnd) &
                     = cld_tau_ssa(ibnd,icol,ilev) &
                     / cld_tau(ibnd,icol,ilev)
               else
                  optics_out%ssa(icol,ilev,ibnd) = 1
               end if
               if (cld_tau_ssa(ibnd,icol,ilev) > 0) then
                  optics_out%g(icol,ilev,ibnd) &
                     = cld_tau_ssa_asm(ibnd,icol,ilev) &
                     / cld_tau_ssa(ibnd,icol,ilev)
               else
                  optics_out%g(icol,ilev,ibnd) = 0
               end if
            end do
         end do
      end do
      ...
      do icol = 1,size(optics_out%tau,1)
         do ilev = 1,size(optics_out%tau,2)
            optics_out%tau(icol,ilev,:) = reordered( &
               optics_out%tau(icol,ilev,:), map_rrtmg_to_rrtmgp_swbands &
            )
            optics_out%ssa(icol,ilev,:) = reordered( &
               optics_out%ssa(icol,ilev,:), map_rrtmg_to_rrtmgp_swbands &
            )
            optics_out%g(icol,ilev,:) = reordered( &
               optics_out%g(icol,ilev,:), map_rrtmg_to_rrtmgp_swbands &
            )
         end do
      end do
   ...
   subroutine get_cloud_optics_lw( &
      ...
      real(r8), dimension(size(iclwp,1), size(iclwp,2)) :: combined_fraction

      ! Temporary variables to hold absorption optical depth
      real(r8), dimension(nlwbands,size(iclwp,1),size(iclwp,2)) :: &
            ice_tau, liq_tau, snw_tau, cld_tau

      integer :: ilev, icol, ibnd, ncol, nlev
      ...
      ! Get liquid optics
      call conley_liquid_optics_lw(ncol, nlev, lambdac, mu, iclwp, liq_tau)

      ! Get snow optics
      if (do_snow_optics()) then
         call mitchell_ice_optics_lw(ncol, nlev, icswp, des, snw_tau)
      end if
   ...
   subroutine merge_optics_lw(tau1, tau2, tau3, frac1, frac2)
      real(r8), intent(in) :: tau1(:,:,:), tau2(:,:,:)
      real(r8), intent(out) :: tau3(:,:,:)
      real(r8), intent(in), optional :: frac1(:,:), frac2(:,:)
      real(r8), dimension(size(frac1,1), size(frac1,2)) :: combined_fraction
      integer :: ilev, icol, ibnd

      if (present(frac1).and.present(frac2)) then
         combined_fraction = max(frac1, frac2)
         do ilev = 1,size(tau1, 3)
            do icol = 1,size(tau1, 2)
               do ibnd = 1,size(tau1, 1)
                  if (combined_fraction(icol,ilev) > 0) then
                     tau3(ibnd,icol,ilev) = ( &
                        frac1(icol,ilev) * tau1(ibnd,icol,ilev) + &
                        frac2(icol,ilev) * tau2(ibnd,icol,ilev)   &
                     ) / combined_fraction(icol,ilev)
                  else
                     tau3(ibnd,icol,ilev) = 0
                  end if
               end do
            end do
         end do
         ...
         do ilev = 1,size(tau1, 3)
            do icol = 1,size(tau1, 2)
               do ibnd = 1,size(tau1, 1)
                  tau3(ibnd,icol,ilev) = tau1(ibnd,icol,ilev) + tau2(ibnd,icol,ilev)
               end do
            end do
         end do
      end if
   end subroutine merge_optics_lw
                 
   !----------------------------------------------------------------------------

   subroutine set_cloud_optics_sw(pmid, cloud_fraction, optics_byband, optics_out)
      
      use mo_optical_props, only: ty_optical_props_2str
      use mcica_subcol_gen, only: mcica_subcol_mask

      real(r8), intent(in) :: pmid(:,:), cloud_fraction(:,:)
      type(ty_optical_props_2str), intent(in) :: optics_byband
      type(ty_optical_props_2str), intent(inout) :: optics_out
      ...
      ncol = size(pmid, 1)
      nlay = size(pmid, 2)
      ngpt = optics_out%get_ngpt()

      ! Allocate array to hold subcolumn cloud flag
      allocate(iscloudy(ngpt,ncol,nlay))
      ...
      call mcica_subcol_mask(ngpt, ncol, nlay, changeseed, &
                             pmid(1:ncol,1:nlay), &
                             cloud_fraction(1:ncol,1:nlay), &
                             iscloudy(1:ngpt,1:ncol,1:nlay))
      ...
      optics_out%ssa(:,:,:) = 1
      optics_out%g(:,:,:) = 0
      do ilev_cam = 1,nlay  ! Loop over indices on the CAM grid

         ! Index to radiation grid
         ilev_rad = ilev_cam + (nlev_rad - nlay)
         ...
         do icol = 1,ncol
            ...
            do igpt = 1,ngpt
               if (iscloudy(igpt,icol,ilev_cam)) then
                  ...
                  optics_out%g  (icol,ilev_rad,igpt) = optics_byband%g  (icol,ilev_cam,ibnd)
               else
   ...
   subroutine set_cloud_optics_lw(pmid, cloud_fraction, optics_byband, optics_out)
      
      use mo_optical_props, only: ty_optical_props_1scl
      use mcica_subcol_gen, only: mcica_subcol_mask

      real(r8), intent(in), dimension(:,:) :: pmid, cloud_fraction
      type(ty_optical_props_1scl), intent(in) :: optics_byband
      type(ty_optical_props_1scl), intent(inout) :: optics_out
      ...
      ! Dimension sizes
      integer :: ngpt, ncol, nlev

      ! Temporary arrays to hold mcica-sampled cloud optics (ngpt,ncol,nlev)
      logical, allocatable :: iscloudy(:,:,:)
      ...
      allocate(iscloudy(ngpt,ncol,nlev))
      ...
      do ilev_cam = 1,nlev
         ...
         ilev_rad = ilev_cam + (nlev_rad - nlev)
